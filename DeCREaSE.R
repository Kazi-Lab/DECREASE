...
CALC_IC50_EC50_DSS = function(
  ...
  tryCatch({
    ...
    if (any(duplicated(mat_tbl$inhibition))) {
      mat_tbl$inhibition <- seq(
        from = 0,
        length.out = length(mat_tbl$inhibition),
        by = 0.01
      ) +
        mat_tbl$inhibition
    }
    ...
    coef_estim["IC50"] <- ifelse(
      coef_estim["MAX"] <= coef_estim["MIN"] |
        coef_estim["IC50"] > max(mat_tbl$dose, na.rm = T),
      max(mat_tbl$dose, na.rm = T),
      coef_estim["IC50"]
    )
    coef_estim["IC50"] <- ifelse(
      coef_estim["IC50"] < 0,
      min(mat_tbl$dose, na.rm = T),
      coef_estim["IC50"]
    )
    coef_estim["IC50"] <- ifelse(
      coef_estim["IC50"] < 0,
      mean(mat_tbl$dose, na.rm = T),
      coef_estim["IC50"]
    )
    coef_estim["IC50"] <- log10(coef_estim["IC50"])
    coef_estim["IC50"] <- ifelse(
      coef_estim["IC50"] < min(mat_tbl$logconc),
      max(mat_tbl$logconc),
      coef_estim["IC50"]
    )
    coef_estim["IC50"] <- ifelse(
      all(mat_tbl$inhibition < 0),
      max(mat_tbl$logconc, na.rm = T),
      coef_estim["IC50"]
    )
    coef_estim["MIN"] <- 0
    coef_estim["MAX"] <- max(mat_tbl$inhibition, na.rm = T)
    min_lower <- ifelse(
      min(mat_tbl$inhibition, na.rm = T) > 0,
      min(mat_tbl$inhibition, na.rm = T),
      0
    )
    ...
    max_lower <- ifelse(
      ...
      coef_estim["MAX"],
      max(mat_tbl$inhibition, na.rm = T)
    )
    max_lower <- ifelse(
      max_lower < 0,
      coef_estim["MAX"],
    ...
    max_upper <- ifelse(
      any(run_avg[-nrow(mat_tbl)] > run_avg[nrow(mat_tbl)]),
      max(mat_tbl$inhibition[run_avg > run_avg[nrow(mat_tbl)]]),
      coef_estim["MAX"]
    )
    max_upper <- ifelse(
      any(mat_tbl$inhibition > max_upper),
      mean(mat_tbl$inhibition[mat_tbl$inhibition > max_upper]) + 5,
      max_upper
    )
    ...
    if (mean_inh_last < 60) {
      if (mean_inh_last > 25) {
        ...
        coef_estim["IC50"] <- max(mat_tbl$logconc, na.rm = T)
      }
    }
    if (mean(mat_tbl$inhibition[1:3], na.rm = T) < 5) {
      coef_estim["IC50"] <- max(mat_tbl$logconc, na.rm = T)
    }
        ...
        ic50std_resid <- round(
          sqrt(
            sum((sumIC50[[1]]$residuals)^2) /
              (length(sumIC50[[1]]$residuals) - 1)
          ),
          1
        )
        ic50std_resid2 <- round(
          sqrt(
            sum((sumIC50[[2]]$residuals)^2) /
              (length(sumIC50[[2]]$residuals) - 1)
          ),
          1
        )
    ...
    if (coef(nls_result_ic50)["SLOPE"] <= 0.2) {
      ...
      nls_result_ic50 <- nls(
        ...
        start = list(
          SLOPE = 1,
          MIN = unname(coef_estim["MIN"]),
          MAX = unname(coef_estim["MAX"]),
          IC50 = unname(coef_estim["IC50"])
        ),
        lower = list(
          SLOPE = 0.1,
          MIN = min_lower,
    ...
    if (readoutCTX) {
      names(coef_tec50) <- c("TC50", "SLOPE", "MAX", "MIN")
      ytec <- yic
      perViaTox <- perInh
    } else {
    ...
    coef_ic50 <- c(
      ...
      Max.Conc.tested = max_signal,
      IC50_std_error = ic50std_Error,
      DSS = dss_score
    )
    coef_tec50 <- c(
      coef_tec50,
      Min.Conc.tested = min_signal,
      Max.Conc.tested = max_signal,
      TEC50_std_error = ic50std_Error
    )
    IC50_df <- data.frame(
      DRUG_NAME = "drug_name",
      ANALYSIS_NAME = "IC50",
      t(as.matrix(coef_ic50)),
      perInh,
      GRAPH = NA,
      DSS = as.numeric(dss_score),
      sDSS = "",
...
for (k in 2:nrow(bliss.mat)) {
  for (j in 2:ncol(bliss.mat)) {
    bliss.mat[k, j] <- bliss.mat[k, 1] +
      bliss.mat[1, j] -
      bliss.mat[k, 1] * bliss.mat[1, j] / 100
  }
}
