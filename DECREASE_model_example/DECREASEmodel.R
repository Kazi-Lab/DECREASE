    ...
    MAD_ <- mclapply(
      ...
      function(repCv) {
        ...
        for (k in 1:length(flds)) {
          ...
          fit = xgboost(
            data = as.matrix(trainData[, c("R1", "R2", "Conc1", "Conc2")]),
            label = trainData$Response,
            verbose = F,
            nrounds = round(2**logNtree),
            nthread = 1,
            save_name = paste0("xgboost", repCv, "_", k, ".model"),
            params = list(
...
modelsXGBoost = mbo(
  ...
  control = setMBOControlInfill(
    setMBOControlTermination(makeMBOControl(), iters = 30),
    crit = makeMBOInfillCritEI()
  )
)$opt.path$env[["path"]]
# order based on error
orderMAD = order(modelsXGBoost$y)
models = modelsXGBoost[orderMAD, ] #run = XGBoostRun[orderMAD]
