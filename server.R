...
shinyServer(function(input, output, session) {
  shinyjs::runjs(
    '$(".toHide2").hide(); $(".toHide5").hide();document.title = "DeCREaSE";'
  )
  ...
  toastr_info(
    paste0(
      "A web-based implementation of method for accurate Drug Combination RESponse prEdictions."
    ),
    title = "Welcome to DeCREaSE!",
    closeButton = !0,
    newestOnTop = !0,
    progressBar = !0,
  ...
  # fit curve
  observeEvent(input$fitcurve, {
    fcurve <<- input$fitcurve
  })
  # docs
  observeEvent(
    input$videobt,
    shinyjs::runjs(
      'window.location.replace("http://decrease.fimm.fi/decrease/decrease_docs/")'
    )
  )
  ...
  observeEvent(input$start, {
    ...
    updateSelectInput(
      session,
      "Graphs",
      choices = paste0(annot$Drug1, " & ", annot$Drug2)
    )
  })
    ...
    if (input$Graphs != "") {
      ...
      if (pair_name %in% list.files()) {
        ...
        output$plotFull <- renderPlot({
          ...
          PlotRespMatr(
            reshape2::acast(finalpred, Conc1 ~ Conc2, value.var = "finalpred_"),
            name = "%inhibition",
            d1N = data_cell_pair$Drug1[[1]],
            d2N = data_cell_pair$Drug2[[1]]
          )$pl
        })
      } else {
      ...
      output$plotSparse <- renderPlot({
        ...
        PlotRespMatr(
          reshape2::acast(data_cell, Conc1 ~ Conc2, value.var = "Response"),
          name = "%inhibition",
          d1N = data_cell_pair$Drug1[[1]],
          d2N = data_cell_pair$Drug2[[1]]
        )$pl
  ...
  observeEvent(input$startanalysis, {
    shinyjs::runjs(
      '$(".toHide3").hide(); $(".toHide4").hide(); $(".toHide5").show();'
    )
        ...
        tryCatch(
          {
            ...
            if (
              ...
              if (tools::file_ext(input$fileinp$name) == 'xlsx') {
                ...
                tools::file_ext(input$fileinp$name) %in% c("txt", "csv")
              ) {
                annot <<- read.table(
              ...
              toastr_info(
                paste0(
                  "DECREASE only support <b>.xlsx</b>, <b>.csv</b>, and <b>.txt</b> file formats!"
                ),
          ...
          error = function(cond) {
            ...
            toastr_error(
              paste0(
                "Something wrong with your file that cannot be handled by application. Please check that <b>\"",
                input$fileType,
                "\"</b> is a correct file format.For more information about input data see <b>section 3</b> in <a style='cursor: pointer;' onclick='javascript:techdoc()'>technical documentation</a>."
              ),
  ...
  observeEvent(input$save_results, {
    ...
    pairsCalculated_ <- as.character(na.omit(sapply(
      ...
      function(k) {
        ...
        if (
          ...
          paste0(combiK$Drug1[[1]], " & ", combiK$Drug2[[1]])
        } else {
          NA
        }
      }
    ...
    showModal(modalDialog(
      ...
      fluidRow(
        ...
        column(
          ...
          offset = 0,
          br(),
          downloadButton("downloadMatrices3", label = "Download (.txt)")
        ),
        column(
          3,
          offset = 1,
    ...
    shinyjs::runjs(
      '$("#downloadMatrices").removeClass("btn-default"); $("#downloadMatrices").addClass("btn-outline-secondary");
  ...
  observeEvent(input$inpfilepreloadtour, {
    ...
    annot <<- openxlsx::read.xlsx(
      "./ExampleData/exampleTabular/exampleTabular.xlsx"
    )
    if (input$phenotypicResponse == "% cell inhibition") {
      annot$Response <<- 100 - annot$Response
    }
    shinyjs::runjs(paste0(
      '$("#fileinp_progress").html(\'<div class="progress-bar" style="width: 100%;">Upload complete</div>\').css("visibility", "visible");
					   $(".form-control").val("',
      name_,
      '");'
    ))
  ...
  observeEvent(input$showcomboplot, {
    source("additionalFunctions.R")
    finalpred = readRDS("examplecombi.RDS")
    finalpred$finalpred_ = 100 - finalpred$finalpred_
    output$plotFull <- renderPlot({
      PlotRespMatr(
        reshape2::acast(finalpred, Conc1 ~ Conc2, value.var = "finalpred_"),
