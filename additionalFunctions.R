...
PlotRespMatr <- function(matr, name = "none", d1N = "", d2N = "") {
  ...
  dose.response.p <- ggplot(data.plot, aes_string(x = "x", y = "y")) +
    geom_tile(aes_string(fill = "Inhibition"), color = "#FCFCFC", size = 1) +
    theme(title = element_text(face = "bold", size = 10)) +
    geom_text(
      aes_string(fill = "Inhibition", label = "Inhibition"),
      size = 3.5
    ) +
    ...
    labs(x = d2N, y = d1N) +
    theme(plot.title = element_text(hjust = 0.5, size = 16)) #+ guides(fill=F)

  dose.response.p <- dose.response.p +
    theme(axis.text.x = element_text(color = "black", face = "bold", size = 11))
  dose.response.p <- dose.response.p +
    theme(axis.text.y = element_text(color = "black", face = "bold", size = 11))
  dose.response.p <- dose.response.p +
...
PlotIntMatr <- function(matr, d1N, d2N) {
  ...
  synergy.score = list()
  synergy.score$drug.pairs = data.frame(
    drug.row = d1N,
    drug.col = d2N,
    concUnit = "nM",
    blockIDs = 1
...
PlotSynergyShiny <- compiler::cmpfun(function(
  data,
  type = "2D",
  graphnumber = 1,
  brushx = NULL,
  ...
  if (graphnumber == 2) {
    ...
    gplot2d <-
      ggplot(plot2d) +
      aes(x, y, z = z, fill = z) +
      geom_raster(interpolate = !0) +
      geom_contour(color = "white", alpha = 0.5) +
      scale_fill_gradientn(
        expression(delta ~ -score),
        colours = myPalette,
        limits = c(start.point, end.point),
      ...
      scale_x_continuous(
        ...
        breaks = seq(
          min(plot2d$x),
          max(plot2d$x),
          by = (max(plot2d$x) - min(plot2d$x)) / (length(x.conc) - 1)
        ),
      ...
      scale_y_continuous(
        ...
        breaks = seq(
          min(plot2d$y),
          max(plot2d$y),
          by = (max(plot2d$y) - min(plot2d$y)) / (length(y.conc) - 1)
        ),
    ...
    if (gridsize2 != 0) {
      gplot2d <- gplot2d +
        geom_vline(
          xintercept = seq(min(plot2d$x), max(plot2d$x), by = byx),
          linetype = "dotted"
        ) +
        geom_hline(
          yintercept = seq(min(plot2d$y), max(plot2d$y), by = byy),
          linetype = "dotted"
        )
      ...
      gplot2d +
        ggtitle(plot.title) +
        coord_cartesian(
          xlim = c(brushx[1], brushx[2]),
          ylim = c(brushy[1], brushy[2])
        ) +
...
calcsyn <- compiler::cmpfun(function(scores, drug.pairs) {
  ...
  color.range <- round(
    max(
      abs(max(as.numeric(combscores), na.rm = !0)),
      abs(min(as.numeric(combscores), na.rm = !0))
    ) +
      5,
    -1
  )
  ...
  scores.dose[nrow(scores.dose), ncol(scores.dose)] <- max(
    scores.dose[nrow(scores.dose) - 1, ncol(scores.dose)],
  ...
  kriged = tryCatch(
    {
      tmp <- cbind(
        expand.grid(c(0:(length(x.conc) - 1)), c(0:(length(y.conc) - 1))),
        c(as.matrix(scores.dose))
      )
      kriging(
        tmp[, 1],
        tmp[, 2],
        tmp[, 3],
        lags = ifelse(dim(scores.dose)[1] < 8, 2, 3),
        pixels = pixels.num,
        model = "spherical"
      )
    },
    error = function(e) {
      ...
      tryCatch(
        {
          m = apply(
            t(apply(scores.dose, 1, function(x) {
              appro(x, ncol(scores.dose) * 2)
            })),
            2,
            function(x) appro(x, nrow(scores.dose) * 2)
          )
          tmp2 <- cbind(
            expand.grid(c(0:(nrow(m) - 1)), c(0:(ncol(m) - 1))),
            c(as.matrix(m))
          )
          kriging(
            tmp2[, 1],
        ...
        error = function(e) {
          m = apply(
            t(apply(scores.dose, 1, function(x) {
              appro(x, ncol(scores.dose) * 3)
            })),
            2,
            function(x) appro(x, nrow(scores.dose) * 3)
          )
          tmp2 <- cbind(
            expand.grid(c(0:(nrow(m) - 1)), c(0:(ncol(m) - 1))),
            c(as.matrix(m))
